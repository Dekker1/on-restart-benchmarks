...
private:
  ...
  public:
    ...
    void doProcess() {
      ...
      MZN_MIPD__assert_for_feas(sDomain.size(), "Clique " << nClique << ": main ref var "
                                                          << cls.varRef1->id()->str()
                                                          << ", domain dec: " << sDomain);
    ...
    void projectVariableConstr(VarDecl* vd, std::pair<double, double> eq1) {
      ...
      if (vd->ti()->domain()) {
        if (vd->type().isint() || vd->type().isfloat()) {  // INT VAR OR FLOAT VAR
          ...
          MZN_MIPD__assert_for_feas(0, "Variable " << vd->id()->str() << " of type "
  ...
  VarDecl* expr2VarDecl(Expression* arg) {
    ...
    if (0 == id) return 0;  // the call using this should be ignored?
    VarDecl* vd = id->decl();
    MZN_MIPD__assert_hard(vd);
    return vd;
  }
