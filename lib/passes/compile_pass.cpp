...
Env* changeLibrary(Env& e, vector<string>& includePaths, string globals_dir,
  ...
  std::stringstream ss;
  for (auto& name : include_names) ss << "include \"" << name << "\";";
  ...
  // std::cerr);
  Model* inc_mod = parseFromString(*fenv, ss.str(), m->filepath().str() + "_Dummy.mzn",
                                   new_includePaths, true, true, verbose, std::cerr, syntax_errors);
  if (inc_mod == nullptr) {
