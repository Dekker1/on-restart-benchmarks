...
int main(int argc, char** argv) {
...

  {
    ...
    Env env;
    if (Model* m = parse(env, filenames, vector<string>(), "", "", includePaths, flag_ignoreStdlib,
      ...
      try {
        ...
        if (flag_rst) {
          docs = RSTPrinter::printRST(env.envi(), m, basename, toplevel_groups, flag_include_stdlib,
                                      flag_index);
        } else {
