...
SolverInstanceBase* NL_SolverFactory::doCreateSI(std::ostream& log,
...
SolverInstance::Status NLSolverInstance::solve(void) {
  ...
  if (opt.do_keepfile) {
    ...
    string file_mzn = _env->envi().orig_model ? _env->envi().orig_model->filepath().str()
                                              : _env->envi().model->filepath().str();
