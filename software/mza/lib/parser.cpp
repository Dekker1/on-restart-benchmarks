...
void parse(Env& env, Model*& model, const vector<string>& filenames,
           ...
           const std::string& modelStringName, const vector<string>& ip, bool ignoreStdlib,
  ...
  if (!ignoreStdlib) {
    ...
    IncludeI* stdlibinc = new IncludeI(stdlibloc, stdlib->filename());
...
Model* parse(Env& env, const vector<string>& filenames, const vector<string>& datafiles,
  ...
  parse(env, model, filenames, datafiles, textModel, textModelName, ip, ignoreStdlib,
        parseDocComments, verbose, err, se);
  return model;
}

Model* parseData(Env& env, Model* model, const vector<string>& datafiles,
                 const vector<string>& includePaths, bool ignoreStdlib, bool parseDocComments,
                 bool verbose, ostream& err) {
  vector<string> filenames;
  std::vector<SyntaxError> se;
  parse(env, model, filenames, datafiles, "", "", includePaths, ignoreStdlib, parseDocComments,
        verbose, err, se);
  return model;
}

Model* parseFromString(Env& env, const string& text, const string& filename,
                       const vector<string>& ip, bool ignoreStdlib, bool parseDocComments,
  ...
  parse(env, model, filenames, datafiles, text, filename, ip, ignoreStdlib, parseDocComments,
        verbose, err, syntaxErrors);
  return model;
}
