...
ArrayLit* b_arrayXd(EnvI& env, Call* call, int d) {
  ...
  if (dim1d != al->size()) throw EvalError(env, al->loc(), "mismatch in array dimensions");
...
bool b_xorall_par(EnvI& env, Call* call) {
  ...
  int count = 0;
  ArrayLit* al = eval_array_lit(env, call->arg(0));
  for (unsigned int i = al->size(); i--;) count += eval_bool(env, (*al)[i]);
  return count % 2 == 1;
}
...
Expression* b_outputJSON(EnvI& env, Call* call) { return createJSONOutput(env, false, false); }
...
std::string b_format(EnvI& env, Call* call) {
  ...
  if (e->type() == Type::parint()) {
    ...
    if (s.size() < std::abs(width)) {
      ...
      for (int i = addLeft; i--;) oss << " ";
      oss << s;
      for (int i = addRight; i--;) oss << " ";
      return oss.str();
    } else {
      return s;
    }
  ...
  struct Ord {
    ...
    bool operator()(Expression* e0, Expression* e1) {
      switch (e0->type().bt()) {
        ...
        case Type::BT_BOOL:
          return eval_bool(env, e0) < eval_bool(env, e1);
        case Type::BT_FLOAT:
