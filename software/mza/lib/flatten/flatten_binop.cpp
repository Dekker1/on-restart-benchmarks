...
ASTString opToBuiltin(BinOp* op, BinOpType bot) {
  ...
  } else if (op->rhs()->type().isopt() && (bot == BOT_EQUIV || bot == BOT_EQ)) {
...
template <class Lit>
void collectLinExps(EnvI& env, typename LinearTraits<Lit>::Val c, Expression* exp,
...
void flatten_linexp_binop(EnvI& env, Ctx ctx, VarDecl* r, VarDecl* b, EE& ret, Expression* le0,
  ...
  if (coeffv.size() == 0) {
    ...
    return;
  } else if (coeffv.size() == 1 && std::abs(coeffv[0]) == 1) {
    if (coeffv[0] == -1) {
      switch (bot) {
        case BOT_LE:
          bot = BOT_GR;
