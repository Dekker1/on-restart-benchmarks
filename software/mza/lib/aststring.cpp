...
ASTStringO::ASTStringO(const std::string& s) : ASTChunk(s.size() + sizeof(size_t) + 1) {
...
ASTStringO* ASTStringO::a(const std::string& s) {
  ASTStringO* as = static_cast<ASTStringO*>(alloc(1 + sizeof(size_t) + s.size()));
  new (as) ASTStringO(s);
  return as;
}
