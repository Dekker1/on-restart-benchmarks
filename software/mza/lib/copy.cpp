...
void CopyMap::insert(Expression* e0, Expression* e1) {
  if (!e0->isUnboxedVal() && !e1->isUnboxedVal()) node_m.insert(e0, e1);
}
...
Expression* copy(EnvI& env, CopyMap& m, Expression* e, bool followIds, bool copyFundecls,
  ...
  switch (e->eid()) {
    ...
    case Expression::E_TI: {
      ...
      TypeInst* c = new TypeInst(copy_location(m, e), t->type(), ASTExprVec<TypeInst>(r),
...
Item* copy(EnvI& env, CopyMap& m, Item* i, bool followIds, bool copyFundecls, bool isFlatModel) {
  ...
  switch (i->iid()) {
    case Item::II_INC: {
      ...
      IncludeI* c = new IncludeI(copy_location(m, i), ASTString(ii->f().str()));
      m.insert(i, c);
    ...
    case Item::II_ASN: {
      ...
      AssignI* c = new AssignI(copy_location(m, i), a->id().str(), NULL);
      m.insert(i, c);
    ...
    case Item::II_OUT: {
      ...
      OutputI* c = new OutputI(copy_location(m, i),
    ...
    case Item::II_FUN: {
      ...
      FunctionI* c = new FunctionI(
          copy_location(m, i), f->id().str(),
