...
typename E::Val eval_id(EnvI& env, Expression* e) {
  Id* id = e->cast<Id>();
  if (id->decl() == NULL) throw EvalError(env, e->loc(), "undeclared identifier", id->str().str());
  VarDecl* vd = id->decl();
  while (vd->flat() && vd->flat() != vd) vd = vd->flat();
  if (vd->e() == NULL)
...
public:
  typedef SetLit* Val;
  typedef Expression* ArrayVal;
  static SetLit* e(EnvI& env, Expression* e) { return new SetLit(e->loc(), eval_intset(env, e)); }
  static Expression* exp(Expression* e) { return e; }
  Expression* flatten(EnvI&, Expression*) {
    throw InternalError("evaluating var assignment generator inside par expression not supported");
  }
...
public:
  typedef SetLit* Val;
  typedef Expression* ArrayVal;
  static SetLit* e(EnvI& env, Expression* e) { return new SetLit(e->loc(), eval_boolset(env, e)); }
  static Expression* exp(Expression* e) { return e; }
  Expression* flatten(EnvI&, Expression*) {
    throw InternalError("evaluating var assignment generator inside par expression not supported");
  }
  ...
  switch (e->eid()) {
    ...
    case Expression::E_ITE: {
      ...
      for (int i = 0; i < ite->size(); i++) {
        if (eval_bool(env, ite->e_if(i))) return eval_array_lit(env, ite->e_then(i));
      }
...
Expression* eval_arrayaccess(EnvI& env, ArrayLit* al, const std::vector<IntVal>& dims,
  ...
  IntVal realidx = 0;
  int realdim = 1;
  for (int i = 0; i < al->dims(); i++) realdim *= al->max(i) - al->min(i) + 1;
  for (int i = 0; i < al->dims(); i++) {
    ...
    if (ix < al->min(i) || ix > al->max(i)) {
      ...
      if (t.isstring()) return new StringLit(Location(), "");
...
IntSetVal* eval_intset(EnvI& env, Expression* e) {
  ...
  switch (e->eid()) {
    case Expression::E_SETLIT: {
      ...
      for (unsigned int i = 0; i < sl->v().size(); i++) vals[i] = eval_int(env, sl->v()[i]);
    ...
    case Expression::E_ITE: {
      ITE* ite = e->cast<ITE>();
      for (int i = 0; i < ite->size(); i++) {
        if (eval_bool(env, ite->e_if(i))) return eval_intset(env, ite->e_then(i));
      }
    ...
    case Expression::E_CALL: {
      ...
      if (ce->decl()->_builtins.e) return eval_intset(env, ce->decl()->_builtins.e(env, ce));
...
FloatSetVal* eval_floatset(EnvI& env, Expression* e) {
  ...
  switch (e->eid()) {
    case Expression::E_SETLIT: {
      ...
      for (unsigned int i = 0; i < sl->v().size(); i++) vals[i] = eval_float(env, sl->v()[i]);
    ...
    case Expression::E_ITE: {
      ITE* ite = e->cast<ITE>();
      for (int i = 0; i < ite->size(); i++) {
        if (eval_bool(env, ite->e_if(i))) return eval_floatset(env, ite->e_then(i));
      }
  ...
  try {
    ...
    switch (e->eid()) {
      ...
      case Expression::E_ITE: {
        ITE* ite = e->cast<ITE>();
        for (int i = 0; i < ite->size(); i++) {
          if (eval_bool(env, ite->e_if(i))) return eval_bool(env, ite->e_then(i));
        }
      ...
      case Expression::E_BINOP: {
        ...
        if (rhs->type().bt() == Type::BT_TOP) rhs = eval_par(env, rhs);
        if (bo->op() == BOT_EQ && (lhs->type().isopt() || rhs->type().isopt())) {
          if (lhs == constants().absent || rhs == constants().absent) return lhs == rhs;
        }
        if (bo->decl() && bo->decl()->e()) {
        ...
        if (lhs->type().isbool() && rhs->type().isbool()) {
          ...
          try {
            ...
            if (al0->size() != al1->size()) return false;
      ...
      case Expression::E_CALL: {
        try {
          ...
          if (ce->decl()->_builtins.e) return eval_bool(env, ce->decl()->_builtins.e(env, ce));

          if (ce->decl()->e() == NULL)
            throw EvalError(env, ce->loc(),
                            "internal error: missing builtin '" + ce->id().str() + "'");
...
IntSetVal* eval_boolset(EnvI& env, Expression* e) {
  ...
  switch (e->eid()) {
    case Expression::E_SETLIT: {
      ...
      if (sl->isv()) return sl->isv();
      std::vector<IntVal> vals(sl->v().size());
      for (unsigned int i = 0; i < sl->v().size(); i++) vals[i] = eval_bool(env, sl->v()[i]);
      return IntSetVal::a(vals);
    }
    case Expression::E_BOOLLIT:
    case Expression::E_INTLIT:
    ...
    case Expression::E_ITE: {
      ITE* ite = e->cast<ITE>();
      for (int i = 0; i < ite->size(); i++) {
        if (eval_bool(env, ite->e_if(i))) return eval_boolset(env, ite->e_then(i));
      }
    ...
    case Expression::E_BINOP: {
      ...
      if (lhs->type().isintset() && rhs->type().isintset()) {
        ...
        if (bo->op() != BOT_DOTDOT)
          throw EvalError(env, e->loc(), "not a set of bool expression", bo->opToString());
        return IntSetVal::a(eval_bool(env, lhs), eval_bool(env, rhs));
      } else {
        throw EvalError(env, e->loc(), "not a set of bool expression", bo->opToString());
      }
    } break;
    ...
    case Expression::E_CALL: {
      ...
      if (ce->decl()->_builtins.e) return eval_boolset(env, ce->decl()->_builtins.e(env, ce));
  ...
  try {
    switch (e->eid()) {
      ...
      case Expression::E_ITE: {
        ITE* ite = e->cast<ITE>();
        for (int i = 0; i < ite->size(); i++) {
          if (eval_bool(env, ite->e_if(i))) return eval_int(env, ite->e_then(i));
        }
      ...
      case Expression::E_BINOP: {
        ...
        switch (bo->op()) {
          ...
          case BOT_POW:
            return v0.pow(v1);
          case BOT_IDIV:
            if (v1 == 0) throw ResultUndefinedError(env, e->loc(), "division by zero");
            return v0 / v1;
          case BOT_MOD:
            if (v1 == 0) throw ResultUndefinedError(env, e->loc(), "division by zero");
            return v0 % v1;
  ...
  try {
    ...
    switch (e->eid()) {
      ...
      case Expression::E_ITE: {
        ITE* ite = e->cast<ITE>();
        for (int i = 0; i < ite->size(); i++) {
          if (eval_bool(env, ite->e_if(i))) return eval_float(env, ite->e_then(i));
        }
      ...
      case Expression::E_BINOP: {
        ...
        switch (bo->op()) {
          ...
          case BOT_DIV:
            if (v1 == 0.0) throw ResultUndefinedError(env, e->loc(), "division by zero");
            return v0 / v1;
          default:
...
std::string eval_string(EnvI& env, Expression* e) {
  ...
  switch (e->eid()) {
    ...
    case Expression::E_ITE: {
      ITE* ite = e->cast<ITE>();
      for (int i = 0; i < ite->size(); i++) {
        if (eval_bool(env, ite->e_if(i))) return eval_string(env, ite->e_then(i));
      }
...
Expression* eval_par(EnvI& env, Expression* e) {
  ...
  switch (e->eid()) {
    ...
    case Expression::E_COMP:
      if (e->cast<Comprehension>()->set()) return EvalSetLit::e(env, e);
      // fall through
    case Expression::E_ARRAYLIT: {
      ArrayLit* al = eval_array_lit(env, e);
      std::vector<Expression*> args(al->size());
      for (unsigned int i = al->size(); i--;) args[i] = eval_par(env, (*al)[i]);
        ...
        case Expression::E_ITE: {
          ...
          for (int i = 0; i < ite->size(); i++) {
            if (ite->e_if(i)->type() == Type::parbool()) {
              if (eval_bool(env, ite->e_if(i))) return eval_par(env, ite->e_then(i));
            } else {
...
public:
  ...
  bool enter(Expression* e) {
    if (e->type().isann()) return false;
    if (e->isa<VarDecl>()) return false;
    if (e->type().dim() > 0) return false;
    if (e->type().ispar()) {
      if (e->type().isint()) {
  ...
  void vCall(Call& c) {
    if (c.id() == constants().ids.lin_exp || c.id() == constants().ids.sum) {
      ...
      for (unsigned int i = al->size(); i--;) {
        ...
        cbi.run((*al)[i]);
        if (!valid) {
          for (unsigned int j = al->size() - 1; j > i; j--) _bounds.pop_back();
          return;
        }
      }
...
public:
  ...
  bool enter(Expression* e) {
    ...
    if (e->isa<VarDecl>()) return false;
    if (e->type().dim() > 0) return false;
    if (!e->type().isintset()) return false;
    if (e->type().ispar()) {
