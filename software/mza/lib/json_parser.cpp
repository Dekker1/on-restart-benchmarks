...
JSONParser::Token JSONParser::readToken(istream& is) {
  ...
  while (is.good()) {
    ...
    if (!is.good()) throw JSONError(env, errLocation(), "tokenization failed");
    switch (state) {
      case S_NOTHING:
        switch (buf[0]) {
          case '\n':
...
Expression* JSONParser::parseObject(istream& is) {
  ...
  if (objid.t != T_STRING) throw JSONError(env, errLocation(), "invalid object");
...
void JSONParser::parse(Model* m, std::istream& is) {
  line = 0;
  column = 0;
  ...
  for (;;) {
    ...
    if (next.t != T_COMMA) throw JSONError(env, errLocation(), "cannot parse JSON file");
  }
}

void JSONParser::parse(Model* m, const std::string& filename0) {
  filename = filename0;
  ifstream is;
...
void JSONParser::parseFromString(Model* m, const std::string& data) {
