...
struct MznModel {
  PyObject_HEAD Env* _e;
  Model* _m;
  vector<string>* includePaths;

  // SolverCode definition is in global.h
  enum SolverCode { SC_UNKNOWN, SC_GECODE } sc;
 ...
 * Return:
 *      None
 * Note:  It is the Python Interface responsibility to check if parsed argument is of type bool or
 * not
 */
...
static PyMethodDef MznModel_methods[] = {
    ...
    {"load_from_string", (PyCFunction)MznModel_load_from_string, METH_KEYWORDS,
     "Load MiniZinc model from standard input"},
    {"addData", (PyCFunction)MznModel_addData, METH_VARARGS, "Add data to a MiniZinc model"},
    {"solve", (PyCFunction)MznModel_solve, METH_VARARGS | METH_KEYWORDS,
     "Solve a loaded MiniZinc model"},
    {"set_time_limit", (PyCFunction)MznModel_set_time_limit, METH_VARARGS,
     "Limit the execution time of the model"},
    {"set_solver", (PyCFunction)MznModel_set_solver, METH_VARARGS,
     "Choose which model will be used to solve the model"},
    {"Declaration", (PyCFunction)MznModel_Declaration, METH_VARARGS,
     "Add a variable into the model"},
    {"Constraint", (PyCFunction)MznModel_Constraint, METH_VARARGS,
    ...
    {"debugprint", (PyCFunction)MznModel_debugprint, METH_NOARGS,
     "Print the MiniZinc representation of the current model"},
    {NULL} /* Sentinel */
};
...
static PyTypeObject MznModel_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "minizinc_internal.Model", /* tp_name */
